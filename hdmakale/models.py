...
class mKategoriler(models.Model):
   KATEGORI_RESIMLERI = (
   ...
   kategori_resim   = models.CharField(max_length=255, verbose_name="Kategori Resmi", help_text = "Kategori Resmi", choices=KATEGORI_RESIMLERI) 
   ...
   def get_absolute_url(self):
	   return "/makale/%s/" %self.slug
	   
   def save(self, *args, **kwargs):
       self.sef_baslik = unidecode(self.baslik)
       super(mKategoriler, self).save(*args, **kwargs)
	   
       self.aciklama_sef = unidecode(self.aciklama)
       super(mKategoriler, self).save(*args, **kwargs) 	 	   

class Makaleler(models.Model):
   baslik    	    = models.CharField(max_length=255, verbose_name="Baslik", help_text = "Makale Basligi")
   sef_baslik	    = models.CharField(max_length=255, verbose_name="Baslik Sef", help_text = "Baslik Sef", blank=True) 
   slug      	    = models.SlugField(max_length=255, verbose_name="Slug")
   
   aciklama    	    = models.CharField(max_length=500, verbose_name="Aciklama", help_text = "Makale Aciklamasi")
   aciklama_sef	    = models.CharField(max_length=500, verbose_name="Aciklama Sef", help_text = "Aciklama Sef", blank=True)
   
   kategori			= models.ForeignKey("mKategoriler", verbose_name="Kategori")
   ...
   def get_absolute_url(self):
	   return "/makale/%s/%s/" %(self.kategori.slug, self.slug)
